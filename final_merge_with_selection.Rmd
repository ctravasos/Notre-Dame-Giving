...
```{r}
merged2 <- merge(merged, old_portfolio %>% select(ID.Number, Region), by = "ID.Number")
```

```{r}
merged2 %>% group_by(Region) %>% summarize(count = n())

merged2_cut <- merged2 %>% filter(Region != "Unknown") %>%
  group_by(Region) %>%
  arrange(Rank_gift, by_group = TRUE) %>%
  slice_head(n = 2000) %>%
  ungroup()

#merged2_cut %>% group_by(Region) %>% summarize(count = n())
```

```{r}
merged2_cut <- merged2_cut %>% group_by(Region) %>%
...
```{r}
old_clean <- old_portfolio %>% select(-one_of(setdiff(names(merged2_cut), "ID.Number")))

merged_df <- merged2_cut %>%
  left_join(old_clean, by = "ID.Number")
merged_df <- merged_df %>% select(-tenure_rank, -Rank_gift)

write.csv(merged_df, "our_new_portfolio.csv")
```
