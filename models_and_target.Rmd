...
Rockne...Current.Year..,Rockne...One.Year.Prior..,Law...Current.Year.., Law...One.Year.Prior.., Grad.Bus...Current.Year.., Grad.Bus...One.Year.Prior.., Current.Parent.Ind, Monogram...Big.4, Monogram...Non.Big.4, Dual.Domer.Household, General.Active.Giving.Society, General.Gift.Designation, Region, Year, Gift.Amount, Region, Age.Group) %>% #select cols
  ...
  mutate(Consecutive.Years = Consecutive.Years + YearsAgo, Age = Age + YearsAgo, Gift.Amount = ifelse(is.na(Gift.Amount), 0, Gift.Amount), X70.5..Ind = ifelse(Age > 70, 1, 0)) %>% #year adjusting stats
...
mean_age <- mean(all_int$Age, na.rm = TRUE)
all_int$Age[is.na(all_int$Age)] <- mean_age

scaled <- all_int %>% select(-ID.Number, -Gift.Amount, -next_gift, -Year, -YearsAgo) %>% select(where(is.numeric)) %>% scale() %>% as.data.frame
...
interaction_model <- randomForest(next_gift ~ Age + Region  + Consecutive.Years + General.Gift.Designation + Current.Parent.Ind + Monogram...Big.4 + Monogram...Non.Big.4 + Dual.Domer.Household + Has.Spouse.Indicator  + Consecutive.Years + Lifetime.Production.Household.Extended.Amount + Current.Parent.Ind + YearsAgo * Gift.Amount, 
...
tenure_model <- randomForest(Consecutive.Years ~.,
                        data = train_data)
```
...
```{r}
target_gift <- merged %>% group_by(Region) %>% arrange(Rank_gift) %>% slice_head(n = 1575)
target_tenure <- merged %>% group_by(Region) %>% arrange(Rank_tenure) %>% slice_head(n = 1575)
...
```{r}
over <- c("Illinois", "NY Metro", "Southern", "Southwest")
...
under <- c("Central", "East Coast", "Northwest")
...
intersect(original_new$ID.Number, old_portfolio$ID.Number)

original_new %>% filter(ID.Number %in% old_portfolio$ID.Number)
```
