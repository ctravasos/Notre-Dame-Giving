...
old_clean <- old_portfolio %>%
  select(
    ...
    Dual.Domer.Household, Last.Gift.Days.Ago
  ) %>%
  na.omit()
...
gls_std <- gls(
  Consecutive.Years ~ Age.Group + Age + Region + General.Gift.Designation +
    General.Active.Giving.Society + Current.Parent.Ind + Monogram...Big.4 +
    Monogram...Non.Big.4 + Dual.Domer.Household + Last.Gift.Days.Ago,
  weights = varExp(form = ~ fitted(.)),
  data = std_data
)
